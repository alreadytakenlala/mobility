...
export default class Mobility {
     ...
     */
    bindMouse() {
        if (this.isBindMouse == true) return;
        this.isBindMouse = true;
        let body = document.querySelector("body");
        let bodyOnmousedown = body.onmousedown;
        let bodyOnmouseup = body.onmouseup;
        let bodyOnmousemove = body.onmousemove;
        body.onmousedown = (e) => {
            if (bodyOnmousedown) bodyOnmousedown();
            if (this.onmousedown) this.onmousedown(e);
        };
        body.onmouseup = (e) => {
            if (bodyOnmouseup) bodyOnmouseup();
            if (this.onmouseup) this.onmouseup(e);
        };
        body.onmousemove = (e) => {
            if (bodyOnmousemove) bodyOnmousemove();
